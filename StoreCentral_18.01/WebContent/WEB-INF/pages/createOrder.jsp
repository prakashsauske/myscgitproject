...
<title>Create Warehouse Order</title>
...
<body>
	...
	<div class="mainWrapper woolworths">
			...
			<div class="breadcrumbWrapper">
				<div class="breadcrumbs">
					...
					<ul>
						...
						<li class="currentPage"><a href='#'>Create Warehouse
								Order</a></li>
					</ul>
				</div>
				<!-- End of breadcrumbs -->
		...
		<div class="contentWrapper orderDetails">
			...
			<div class="ContentTableWrapper">
				...
				<div class="tableInfo">
					...
					<div class="tableTitle">
						<h4 class="sectionTitle">Create a Warehouse Order with
							Articles</h4>
					</div>
					<!-- End of table title -->
						...
						<div class="formWrapper">
							...
							<div class="parameter">
								<label for="article" class="mandatory">Article</label> <input
									type="#" class="textbox articleSearchText" id="articleNo"
									name="articleNo" maxlength="20" tabindex="1"
							...
							<div class="parameter">
								<label for="sourceOfSupply" class="">Source of Supply</label> <input
									type="radio" name="sourceSupply" tabindex="3" value="2"
									id="warehouse" checked><label for="warehouse"
									class="labelText">Warehouse</label> <input type="radio"
									name="sourceSupply" tabindex="4" value="1" class="hideBlock"
									id="vendor"><label for="vendor" style="display: none"
									class="labelText">Vendor</label>


								<div class="parameter IBTSource">
									<span id="warehouseField"> <select class="selectOptions"
										id="warehouseDropdown" name="warehouseDropdown">
										...
										type="#" class="textbox" name="suppName" value=""
										id="supplier" maxlength="20"
					...
					<table cellspacing="0"
						...
						<c:forEach items="${articleList}" var="articleSearchResults">
							<tr id="row-1" class="row-1" data-tt-id="<%=nodeId%>">
								...
								<td id="qty-<%=j%>" class="numberColumn hideBlock nullCheck ">${articleSearchResults.inputQty}</td>
			...
			<div id="dialog-cancelOrder" title="Create Order">
				<div class="popupContent">
			...
			<div id="dialog-modal" title="Create Order">
				<div class="popupContent">
	...
	<div id="dialog-mulipleArticles" title="Select Articles">
		<div class="popupContent">
			...
			<div class="popupData popupTitle">
				...
				<h4 class="warning">
					Selected supplier: <strong>Olive Supplier Ltd.</strong>
				</h4>
			...
			<div class="" id="popupDataDiv2"></div>
			<!-- End of pop up data -->
			<div class="popupActionsWrapper">
				<label><strong>Note:</strong> You need to create an order
					for single supplier.</label> <span class="popupActions"> <label
					class="secondaryActionBtn" onclick="">Cancel</label> <label
					class="actionBtn addToList">Add to Order List <label
						class="list-count"></label></label>
	...
	<script>
		...
		$(".saveRowBtn").click(function(){
			...
			var index=id;
			var flag=true;
			if($(("#rosterDateValue-").concat(id)).val().split('/').length==3 && $(("#deliverDateValue-").concat(id)).val().split('/').length==3)
			{
			...
			else if($(("#rosterDateValue-").concat(id)).val().split('/').length!=3)
			{
				$('#alertBox').text('Please enter a valid date');
				$( "#dialog-modal" ).dialog( "open" );
				$('#okBtn').click(function(e){
		...
		/* if(srcSupply=='' || srcSupply=='Enter supplier no. or name' )
			...
			{
			//alert("list is empty");
			$('#alertBox').text('There is no article to be received. Cannot finalise the order');
			$( "#dialog-modal" ).dialog( "open" );
			$('#okBtn').click(function(e){
				$( "#dialog-modal" ).dialog( "close" );
				});;
		...
		$('#okBtn').click(function(e){
			...
			$.ajax({
				...
				success : function(response) {
					...
					$("#con-final").css('display','none');	
					$('#alertBox').text('Order created successfully with the reference #'+message);
					$( "#dialog-modal" ).dialog( "open" );
					$('#okBtn').click(function(e){
						$( "#dialog-modal" ).dialog( "close" );
						$("#manualOrderSearchSubmit").attr('action','onPageLoadCreateManualOrder.htm');
								...
								$('#nodataMsg').text('');
								 ...
								 if($('#sizeCheck1').val()>1){
									 if(!$( "#dialog-modal2" ).dialog( "isOpen" )){
										...
										$("#dialog-modal2").parent().addClass("popupWrapper");			
										$("#dialog-modal2" ).dialog( "open" );
										$("#searchWarning,.warning").addClass('hideBlock');
										$("#popupSearch").removeClass('hideBlock');
										}
								 }
								 else{
		...
		$( "#dialog-modal2" ).dialog({				
			...
			$("#dialog-modal").parent().addClass("popupWrapper");
			 ...
			 $( "#dialog-modal-orders" ).dialog({				
				...
				//New change for roster and delivery Dates
				

				$(".delivery-date").mousedown(function(e) {
					tempDate = $(this).val();
					tempId = $(this).attr('id');
				});
				$(".delivery-date").change(function(e) {
					$("#dialog-date-delivery").dialog('open');
				});

				$(".roster-date").mousedown(function(e) {
					tempDate = $(this).val();
					tempId = $(this).attr('id');
				});
				$(".roster-date").change(function(e) {
					$("#dialog-date-roster").dialog('open');
				});
