	...
	$.ajax({
	    ...
	    beforeSend: function() {
		  ...
		  if(response != undefined  && response.length > 0 && response[0].pcd_id != undefined){
			  ...
			  for(var i=0; i< response.length; i++)
			  //Defect_12500
			  addRowToULDTable(response[i].pallet_type, response[i].pallet_type, response[i].pallet_qty, '', response[i].pcd_Id, $('#addULDTable'),'00');	
		  }		
	  }).fail(function() {	
...
function addToULDTable(srArea, toAddArea) {
	...
	addRowToULDTable(uldId, selectedElem.text(), '', retQty, '', toAddArea,plt_type);
...
//Defect_12500
function addRowToULDTable(uldId, uldType, recQty, retQty, pcdId, toAddArea,plt_type){
	var tempTr = '';
...
//Defect_12500
		tempTr = '<tr plt_type="'+plt_type+'" id="row-' + uldId + '" pcdId="'+pcdId+'"><td>' + uldType
				+ '</td><td class="centerValue" id="uom-1"><input type="#" class="textbox xsmallbox retQty qtyField"  maxlength = "3" value="'+retQty+'">'
