...
var roleMap={'MA':'MA'};
var maxPromoCount = 5;
var currPromoCount = 5;
var currPromoArticles={};
var currPromoArticlesList=[];
var countValidation = false;
var roleId;
var currentSalesOrg = '';
var markDownPercent = '';
var dmSalesOrg = '1015';
var clr_promo_type = 'ZCLR';
var constantsMap = {};
var popUpErrorMsgDiv = '<h4 class="errorAddtnlDtls" style="padding-left:15px; width:60%; /* margin-top:20px; */color:red;background:url(../../images/iconError.png) 0 3px no-repeat;/* display: inline; */display: inline-block;float: left;">Please select atleast one article to add.</h4>';
var currentPageInPast = 1;
var currentPageInActive = 1;
$(document).ready(
		function() {
			currentSalesOrg = $('#salesOrg').val();
			roleId = $('#roleId').val();
			...
			$('#addtolist').click(function(event) {
				...
				hideErrorinPopup($popup);
				var $draft = $('#promoList .drillsOpenDefault.headRow');
				var draftCnt =0;
				if($draft!=undefined && $draft.length>0){
						draftCnt =$draft.length;
				}				
				if ($('input[name="articlecheckbox"]:checked').length >= 1) {
					$elem = $('#promoList .drillsOpenDefault.editRow');
					data = $('input[name="articlecheckbox"]:checked');
					/*if($elem!=undefined && (maxPromoCount<getCurruntPromotionCount()) && countValidation){
						showErrorinPopup($popup,'You cannot have more than '+maxPromoCount
								+' articles on an instore-created promotion per week');	
						//event.preventDefault();
					}else if($elem!=undefined /*&& data.length > (maxPromoCount- currPromoCount) && countValidation){
						newData = data.splice(0,(maxPromoCount- currPromoCount - draftCnt));
						data =data.splice((maxPromoCount- currPromoCount - draftCnt),data.length);
						//var articleNotAdded ='';
						/*for(var i =0;i<data.length ;i++){
							if(articleNotAdded)articleNotAdded += $(data[i]).closest('tr').attr('data-parentno');
							else
							articleNotAdded += ','+$(data[i]).closest('tr').attr('data-parentno');
							}
						addArticleToPromoList(newData,(' Some of the articles not added to draft, You cannot have more than '+maxPromoCount
								+' articles on an instore-created promotion per week'));
						$('#error-message-head').addClass('hideBlock');
						$("#dialog-mulipleArticles").dialog("close");
					}else{*/
						addArticleToPromoList(data,'');
						$("#dialog-mulipleArticles").dialog("close");
					//}
				} else {
					showErrorinPopup($popup,'Please select atleast one article to add.');
					event.preventDefault();
				}
			...
			.click(
					function() {
						...
						$elem = $('#promoList .drillsOpenDefault.headRow');
						if (!$('#article').required()) {
						...
						/*else if($elem!=undefined && ($elem.length >= (maxPromoCount -currPromoCount) || currPromoCount >=maxPromoCount ) && countValidation){
							showAllErrors('You cannot have more than '+maxPromoCount
									+' articles on an instore-created promotion per week');	
							$('#error-message-head').addClass('hideBlock');
						}*/ else {
							$('#article').removeClass('errorField');
			...
			$(".popupActions .secondaryActionBtn")
					.click(function() {
						$("#dialog-mulipleArticles").dialog("close");
			...
			$("#beforePublishBtn").click(
					function() {
						...
						var len= 0;
						var cnt =0;
						var $elem = $(".promolistcheckbox:checked");
						len = getCount($elem.closest('tr'));
						if (len == 0) {
						...
						if($elem!=undefined && (maxPromoCount<getCurruntPromotionCount()) && countValidation){
							cnt = (len -(maxPromoCount - currPromoCount));
								showAllErrors('You cannot have more than '+maxPromoCount
										//+' articles on an instore-created promotion per week, Please remove '+ ((cnt>1) ? cnt+' articles' : cnt+' article') +' from draft');
										+' articles on an instore-created promotion per week');
								$('#error-message-head').addClass('hideBlock');
								return false;
						}
...
//				var param= { "ARTICLE_INFO":$(this).val() };
//			
//				$.post( searchArticleURL,JSON.stringify(param))
//				  .done(function( data ) {
//					//console.log('data '+data);					    
//				    var response=data;
//					var result=[];
//					for(var i=0;i<response.length;i++){
//						result.push(response[i].MATNR+'-'+response[i].MEINS+'-'+response[i].MATKX);
//					}						
...
//			});
			...
			populateDepartment();
			if(dmSalesOrg == currentSalesOrg){
				countValidation = true;
				/*var param = {user_id :currentUser,promo_type : clr_promo_type};
				callServicePromoCount(JSON.stringify(param));*/
			}
			getConfigData();
			securityMatrix();
});
function validatePromoCount(msg){
	if(dmSalesOrg == currentSalesOrg){
		countValidation = true;
		var param = {user_id :currentUser,promo_type : clr_promo_type};
		callServicePromoCount(JSON.stringify(param), msg);
	}
}
function callServicePromoCount(data,msg){
	console.log('URL getPromoCount = '+ getPromoCount);
	console.log('Input getPromoCount = '+ data);
	$.ajax({
		type : "post",
		url  : getPromoCount,
		data : (data),
		...
		success : function(response) {
			if(response!=null && response!=undefined && response.length > 0 
				...
				currPromoArticlesList=response;
				currPromoArticles=$groupBy(response,function(obj) {
					return obj.article;
				});
				currPromoCount = Object.keys(currPromoArticles).length;
				if(msg == ''){
					showPromoCntError();
				}
			}else{
				...
				if(response!=undefined && response.length > 0 && response[0].ErrorID!=undefined){
					//showAllErrors(response[0].ErrorMsg);
					showAllErrors('Technical issue occured in SQL Anywhere, While getting promotion limit');
				}else{
					//There are no exisiting promotion.
...
function showPromoCntError(){
	var $elem = getCount($('#promoList .drillsOpenDefault.editRow'));
	var len = 0;
	var cnt=0;
	if(currPromoCount < maxPromoCount){
		len =$elem.length;
		if($elem!=undefined && (len > (maxPromoCount - currPromoCount))){
			cnt = (len -(maxPromoCount - currPromoCount));
			showAllErrors('You cannot have more than '+maxPromoCount
					//+' articles on an instore-created promotion per week, Please remove '+ ((cnt>1) ? cnt+' articles' : cnt+' article') +' from draft');
					+' articles on an instore-created promotion per week');
		...
		$('#listOption').prop('disabled',true);
		//$('#searchAndAdd,#filterOpen').unbind('click');
		//$('#article').prop('readonly',true);
		$('#promoList #promoListTbody tr').remove();
		$('#promoList,#beforePublish').addClass('hideBlock');
		showAllErrors('You cannot have more than '+maxPromoCount
				+' articles on an instore-created promotion per week');
		$('#error-message-head').addClass('hideBlock');
	}
	...
	$.ajax({
		...
		success : function(response) {
			...
			if (response.length > 1) {
				...
				hideErrorinPopup($('#dialog-mulipleArticles'));
...
function getArticleInfo(data) {
	...
	$.ajax({
		...
		success : function(response) {
			...
			if (message == 'success' && dataObj!=undefined ) {	
...
function getTr(obj) {
	...
	var tr = '<tr data-om="'
			...
			//+
			+ '" data-sellOutDays="'
			+ (obj.sell_out_days  != null && obj.sell_out_days  != undefined ? 
					obj.sell_out_days  : '')
			+'" ><td id="articleNo">'
			+ articleNo
...
function addArticleToPromoList(data,msg) {
	var drafts=[];
	var draftArray=[];
	var alcFailedItems=[];
	var psArticleMap={};
	for(var i=0;i<data.length;i++){
				var rowObj = $(data[i]).parent().parent();
				...
				var psStatus = rowObj.attr('data-articleStatus');
				...
				var artSellOutDays = rowObj.attr('data-sellOutDays'); //Dan Murphy validations
				var psArtcleArray=[];
				var flag =true;
				if($("#currentBanner").val() =='danmurphy' && alcStatus != "DA" && psStatus != "CA" && psStatus != "2" && psStatus != "02"
					&& (isNaN(artSellOutDays)|| artSellOutDays < maxSellOutDays)){
					showError('Article is due to sell out in '+((artSellOutDays==undefined||artSellOutDays==null||artSellOutDays=='')?'0':artSellOutDays.toFixed(2))+' days');//Dan Murphy validations
					flag = false;
				} else
...
function addSingleArticleToPromoList(obj) {
	...
	var artSellOutDays=obj.sell_out_days; //Dan Murphy validations
	...
	var psStatus = (obj.ps_article_status  != null && obj.ps_article_status  != undefined ? 
			obj.ps_article_status  : '');
	var alcStatus = (obj.alc_status  != null && obj.alc_status  != undefined ? 
			obj.alc_status  : '');
	 //Dan Murphy validations	
	if($("#currentBanner").val() =='danmurphy' && (isNaN(artSellOutDays)|| artSellOutDays < maxSellOutDays) &&
		alcStatus != "DA" && psStatus != "CA" && psStatus != "2" && psStatus != "02"){
		showError('Article is due to sell out in '+((artSellOutDays==undefined||artSellOutDays==null||artSellOutDays=='')?'0':artSellOutDays.toFixed(2))+' days'); 
		//show message 'Article is due to sell out days'
	}
	else{
	if ($('#row-' + id).length == 0) {
		$('#promoList #promoListTbody').append(
...
function getPromoItemAsHTML(user,articleNo, description, uom, id, supplier, om,
	...
	if (isBigw == 'true')
		diptype = '<td id="advDisplay-'
				...
				+ adType + '</select></td>';*/
	var row = header 
			+'<tr data-article = "'+articleNo+'" id="row-'
			+ id
			+ '" data-tt-id="'
			+ id
			...
			+ id
			+ '" class="centerValue columnDivider hideBlock"><input maxlength="10" type="text" class="textbox textboxDefaultText inputDate editDateCell " placeholder="dd/mm/yyyy" id="end-'+id+'" value="'
			+ endDate
...
function showAllErrors(content) {
	$('#error-message-head').removeClass('hideBlock');
	...
	if(endreqvalid && streqvalid)
	 ...
	 endNoPast = obj.find('td[data-bind="promoEndDate"] input').noPastValidation();
		...
		/*	}
			...
			{
			flag = obj.find('td[data-bind="promoStartDate"] input').startEndValidationForOtherPromo(obj.find('td[data-bind="promoEndDate"] input'));
		/*	}*/ //commented as the security matrix is changed
		}
		}else{
			flag=false;			
		}
...
function validateNewPrice(obj) {
	...
	if(isDesiredRole(roleMap,roleId))
		...
		newPriceValid = obj.find('td[data-bind="newPrice"] input[type="text"]').newPriceValidation(obj.find('td[data-bind="standardPrice"]')
		...
		if($("#currentBanner").val()=='danmurphy')
			{
			if(flag)
				{
				var stdPriceInt = parseFloat(obj.find('td[data-bind="standardPrice"]').text());
				var newPricePercent = parseFloat(obj.find('td[data-bind="newPrice"] input[type="text"]').val());
				var newPrice = newPricePercent;
				
				if(Number((((stdPriceInt - newPrice) / stdPriceInt) * 100)
						.toFixed(0)) > Number(markDownPercent).toFixed(0))
					{
	...
	rowList.each(function() {
		...
		/*	if(competitorValue == 'Select')
				...
				obj.find('td[data-bind="competitor"] select').change(function() {
					...
					/*if (competitorValue == 'Others') {
						if (comments == '') {

							obj.find('td[data-bind="competitor"]').closest('tr').next().find('.notes input').addClass(errorFieldClass);
							addtooltip(obj.find('td[data-bind="competitor"]').closest('tr').next().find('.notes input'),
									"Mandatory Field.");
							obj.find('td[data-bind="competitor"]').closest('tr').next().find('.notes input').change(function() {
								obj.find('td[data-bind="competitor"]').closest('tr').next().find('.notes input').removeClass(errorFieldClass);
								removetooltip(obj.find('td[data-bind="competitor"]').closest('tr').next().find('.notes input'));
								commentsValidate=false;
								//obj.find('a[title="Expand"]').trigger('click');
							});
							if(!obj.find('a[title="Expand"]').parent().parent().hasClass('collapsed'))
							obj.find('a[title="Expand"]').trigger('click');
						}
					}*/
		var freshFoodFlag = (obj.attr('data-freshFoodFlag') || '');
		if(streq && endreq &&  streqvalid && endreqvalid && streqvalidpast && endreqvalidpast && newPriceReq && newPriceValidate && commentsValidate &&competitorValidate){
			if(freshFoodFlag == 'Y' && freshFoodDayLimit != null && freshFoodDayLimit != '')
				{
				flag = obj.find('td[data-bind="promoStartDate"] input').freshFoodDateValidation(obj.find('td[data-bind="promoEndDate"] input'));
				}
...
function callServiceForValidation(data) {
	...
	$.ajax({
		...
		success : function(response) {
			...
			if (message == 'success'){ 
				if(dataObj!=undefined && dataObj.inStorePromoArticleInfoList!=undefined) {			
...
//		}
	...
	if(errors==''){
		var content='Clearance Price is excessive for article ';
	...
	if(dataObj.msg!=null&&dataObj.msg.trim()!=''&&dataObj.msg!=undefined && dataObj.msg!='success'){
		...
		for(var i=0;i<dataObj.inStorePromoArticleInfoList.length;i++){
			...
			if(item.promoCreateStatus=='Y'){
				...
				$('#row-'+id).find('.status').html(failed);
				$('#row-'+id).find('.status').find('.failed').attr('title','Failed to create promotion.');
				var parentNo = $("#row-" + id).attr('data-parentNo');
				errors+=getError(article,"Failed to create promotion.");				
			}
			
		}
		removeFromDraft(toremoveList,'SUBMITTED');
...
function getActiveAndFuturePromotions(pageNo)
{
	...
	$.ajax({
		type : "get",
		url : "getActiveAndFuturePromotions.htm",
		data : 
			{
			pageNo : pageNo,
			records : 10
		...
		success : function(response) {
			...
			else if(message== 'noData')
			{
				...
				message='No active or future clearance promotion exists.';
				//console.log(message);
...
/*	
	parentElem.find("#startDateEdit-" + id).find('input').change(function(){
		setTimeout(function(){ 
			var flag=validateStartAndEndDate(parentElem.find("#"+rowId+"-" + id));		
		}, 100);
		
	});
	
	parentElem.find("#endDateEdit-" + id).find('input').change(function(){
		setTimeout(function(){ 
			validateStartAndEndDate(parentElem.find("#"+rowId+"-" + id));
		}, 100);
	});
	*/
	...
	$.ajax({
		...
		success : function(response) {
			...
			if (message == 'success') {
				...
				message='No clearance promotion found for search criteria.';
				//console.log(message);
...
function getPromoItemAsHTML4Search(articleNo, description, uom, id, supplier,
	...
	deactivateProperty = createClearance;
	if(stdPrice == null || stdPrice == '' || stdPrice == '0.00' || stdPrice == '0' || isNaN(stdPrice))
	...
	var row =	header
			+'<tr data-article = "'+articleNo+'" id="'+rowId+'-'
			+ id
			+ '" data-tt-id="'
			+ id
...
function callServiceForUpdate(id,elem,rowId){
	...
	/*toDeactivate.start_date=elem.find('#startDateEdit-'+id).find('input').val();
	toDeactivate.end_date=elem.find('#endDateEdit-'+id).find('input').val();*/
	$.ajax({
		type :"post",
		url  :"updatePromo.htm",
		contentType :"application/json",
		...
		success : function(response) {
			...
			if (message == 'success') {
				...
				/*elem.find(("#startDateEdit-").concat(id)).addClass('hideBlock');
				elem.find(("#startDate-").concat(id)).removeClass('hideBlock');
				
				elem.find(("#startDate-").concat(id)).text(elem.find(("#startDateEdit-").concat(id)).find('input').val());
				elem.find(("#endDate-").concat(id)).text(elem.find(("#endDateEdit-").concat(id)).find('input').val());
				
				elem.find(("#endDateEdit-").concat(id)).addClass('hideBlock');
				elem.find(("#endDate-").concat(id)).removeClass('hideBlock');*/
			} else if(message == 'logout'){
				gotoHomeScreenSessionExpired();
			}
			else {
...
function removeFromDraft(listObj,msg){
	console.log("To add List:"+JSON.stringify(listObj));
	var draftArray=[];
	var mode='';var stat = '';
	for(var i=0;i<listObj.length;i++){
...
function loadMyDraft(myDraftArrayList) {
	...
	for ( var i = 0; i < myDraftArrayList.length; i++) {
		if (myDraftArrayList[i].PROMO_TYPE == 'ZCLR') {
	...
	if (myDraftArray != undefined) {
				...
				if (myDraft[i].PROMO_TYPE == 'ZCLR') {
					...
					$('#promoListTbody').append(
							getPromoItemAsHTML(user,articleNo,
									description, uom, id, '', '',
									startDate, endDate, dispType, '', '','',parentNo,status,department,freshFoodFlag));
					$("#row-" + id).find("#dispType").val(dispType);
		...
		showPromoList();
		shrunkTable();
		validatePromoCount('ONLOAD');
		$('#promolistcheckboxall').trigger('click');
	}
}

function modifyDraft(dataObj,msg){
	...
	$.post( modifyMyDraftURL,JSON.stringify(param))
	  .done(function( data ) {
		...
		var msg1 = 'SUBMITTED';
		if(dataObj!=null && dataObj.length >0 && dataObj[0].STATUS != 'SUBMITTED'){
			msg1 = '';
		}
	    validatePromoCount(msg1);
	});
}
	  ...
	  .done(function( result ) {
	    if(result!=undefined){			
			for(var i=0;i<result.length;i++){				
				...
				//$('#row-'+id).find('[data-bind="standardPrice"]').attr('data_cost_price',('3'));
//				$('#row-'+id).find('[data-bind="promoPrice"]').text((result[i].standard_price==null||result[i].standard_price==undefined)?'0':result[i].standard_price.toFixed(2));
	    ...
	    var newParam = {"iv_sales_org": $('#salesOrg').val(),"iv_role_id":roleId}; ////Dan Murphy validations
	    if($("#currentBanner").val()=='danmurphy')
	    	{
	    	console.log(getMarkdownPercentUrl+ JSON.stringify(newParam));
	    	$.post( getMarkdownPercentUrl,JSON.stringify(newParam))
	  	  .done(function( result ) {
	  	    if(result!=undefined){
	  	    	if(result[0] != undefined && result[0].markdown_percentage != undefined)
	  	    		{
	  	    		markDownPercent = result[0].markdown_percentage;
	  	    		}
	  	    }
				stopLoading();
	  	});
...
function validateNextDeliveryDateNExistance(dataObj) {
	...
	for ( var i = 0; i < dataObj.inStorePromoArticleInfoList.length; i++) {
		...
		//var id = obj.articleNo;
		console.log('To remove item ' + id);
		if (obj.promoOfferNo != null && obj.promoOfferNo != "" && obj.promoOfferNo != undefined) {
			existList.push(obj);
	...
	if (currentSalesOrg == dmSalesOrg && existList.length > 0) {
		var message = '';
		// var articleList = [];
		...
		message = 'Article ' + message.substr(0, message.length-1) + "  already on active/future promotion.";
		if(existList)
		message += '';
		showInformation(message);
		return false;
...
function showErrorinPopup(elem, msg) {
	...
	if(msg!=''){
		$(elem).find('.popupActionsWrapper .errorAddtnlDtls').text(msg);
		$('#error-message-head').addClass('hideBlock');
	}
}
		...
		success : function(response) {
			if(response!=null && response!=undefined && response.length > 0 
				...
				if(!isEmptyMap(constantsMap))
					...
					maxSellOutDays =constantsMap["MAX_SELL_OUT_DAYS"];
					maxPromoCount =constantsMap["MAX_NO_OF_ARTICLES_FOR_ZCLR"];
					//maxPromoCount=3;
					}else{
						maxSellOutDays =0;
					}
...
function getCount($elem){
    ...
    $elem.each(function(){
        var $tempElem = $(this);
        var article = $tempElem.attr('data-article');
        if(!tempMap.hasOwnProperty(article)){
            tempMap[article]= article;
            count++;
        }
    });
	return count;
}
function checkIfArticleInPromo(obj){
	var flag=true;
	if(currPromoArticles[obj.article]!=undefined){
		flag=false;
		var list=currPromoArticles[obj.article];
		for(var i in list){
			if(list[i].article_uom==obj.article_uom){
				//ignore
				flag=true;
			}
		}
...
function getCurruntPromotionCount(){
	try{
		var currentPromoArticlesList=$groupBy(currPromoArticlesList,function(obj) {
						return obj.article;
		});
		currPromoCount= Object.keys(currentPromoArticlesList).length;
		
		var list=$groupBy(getInlineList(),function(obj) {
						return obj.article;
		});
...
function getInlineList(){
	try{
		var res=JSON.parse(JSON.stringify(currPromoArticlesList));
		var inList=undefined;
		inList=$.parseJSON(getPostDataObj4ArticleInfoStr($('#promoList')));
		for(var itm in inList){
			res.push({"article":inList[itm].articleNo,"article_uom":inList[itm].articleUom});
		}
		return res;
	}catch(err){
		return JSON.parse(JSON.stringify(currPromoArticlesList));
