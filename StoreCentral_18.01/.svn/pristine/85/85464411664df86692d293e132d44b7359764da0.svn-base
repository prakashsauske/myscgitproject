				...
				if (msg != undefined && msg == '' && !isNaN(msg)
					...
					if (salesSummary != null && salesSummary != "") {
						...
						for ( var i = 0; i < list.length; i++) {
							...
							if (list[i].date.indexOf(ageDate) != -1
									...
									&& list[i].time.toUpperCase().indexOf(ageTime) != -1
...
function ageVerifDetlPrintJasper() {
	if(isNotJasperPrintValid()) {
		printJasperValMsg();
	}
	else {
		$('#ageVerificationDetails').attr("action", "getAgeVerificationDetail.pdf");
		$('#ageVerificationDetails').attr('target','_blank');
		$('#ageVerificationDetails').submit();
	}
}
$( document ).ready(function() {
