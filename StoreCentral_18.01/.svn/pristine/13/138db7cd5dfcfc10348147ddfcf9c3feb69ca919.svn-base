	...
	<div class="ContentTableWrapper">
		<table class="ContentTable" cellspacing="0">
			...
			<c:forEach items="${vendorList}" var="vendorList">
				...
				<tr>
				<input type="hidden" id="vnSuppNo<%=i%>" value="${vendorList.vnVendorNo}" />
					<td id="suppNo<%=i%>">${vendorList.supplierNo}</td>
					<td id="suppName<%=i%>">${vendorList.supplierName}</td>
					<td>${vendorList.telephone}</td>
					<td>${vendorList.fax}</td>
					<td>${vendorList.city}</td>
					<td>${vendorList.region }</td>
					<td>${vendorList.leadTime}</td>
...
<Script>
		//$(document).ready(function(){
			...
			$(".linkBtn1").click(function() {
				...
				$("#supplier").val($("#suppNo"+id+"").text()+"-"+$("#suppName"+id+"").text());
				$("#suppNo").val($("#suppNo"+id+"").text());
				$("#vnVendorNumber").val($("#vnSuppNo"+id+"").val());// defect no 14595
