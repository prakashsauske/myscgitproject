...
$(document).keypress(function(event) {
    if (event.which == 13) {
        ...
        $('.verify').click();
        	...
        	$( "#dialog-confirm #yes:visible").click();
        }else if($("#dialog-confirmation_forPrimary").dialog('isOpen')){
        	$( "#dialog-confirmation_forPrimary #ok:visible").click();
        }else{
        validate();
        }
        }
...
<!-- HostName :  <%=java.net.InetAddress.getLocalHost().getHostName()%> -->
	<form:form method="post" id="loginForm" action="home.htm">
		<input type="hidden" id="confirmSecondaryLogin" value="${confirmSecondaryLogin}"/>
		<input type="hidden" id="userSalesOrg" value="${user.salesOrg}"/>
		<div
	...
	<%@include file="forgotPassPopups.jsp"%>
	<%@include file="verifyStore.jsp"%>
