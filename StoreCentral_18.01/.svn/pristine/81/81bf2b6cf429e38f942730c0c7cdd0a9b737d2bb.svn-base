...
$(function() {	
			...
			$("#export").click(function(){
				$('#dialog-editFunctions .ContentTable tbody').html('');
				exportSql();
			});
...
function checkpoint(){
	var exportQueryURL="backupCheckpoint.htm";
	$.ajax({
	    type: "GET",
	    url: exportQueryURL,
	    ...
	    beforeSend: function() {
		  ...
		  responseO = $.parseJSON(response);
			...
			if(responseO != undefined && responseO.type=='Y' ){
				...
				downloadCSV({"filename":"app_checkpoint.sql","columnDelimiter":";","lineDelimiter":";\n"},qryListObj);
			}else{
...
function verifyAndAdd(){
	var storeslist=$(".verifyNm").val();
	if(storeslist.trim()!='' && storeslist.split(',').length>0){
		searchParam.storeslist=storeslist;
		callVerifyService(searchParam);
		$(".verifyNm").val('');
		$("#dialog-editFunctions .errorDiv ").addClass('hideBlock');
	}else{
		$("#dialog-editFunctions .errorDiv ").removeClass('hideBlock');
	}
};
...
function multipleStore() {
	...
	$("#dialog-editFunctions .ContentTableWrapper").removeClass('hideBlock');
	$("#dialog-editFunctions .ContentTable").addClass('hideBlock');
	$("#dialog-editFunctions #createOrderButton,#exportmultistoreButton").addClass('hideBlock');
	$("#dialog-editFunctions .errorDiv ").addClass(
	'hideBlock');
	$('#createOrderButton').unbind('click');
	$('#createOrderButton')
			.click(
					function() {
						$("#dialog-editFunctions .errorDiv ").addClass('hideBlock');
						if ($('#dialog-editFunctions .ContentTable tbody tr').length > 1) {
							saveStore();
						} else {
							$("#addActionBtn").trigger('click');
							$("#dialog-editFunctions .errorDiv ").removeClass(
									'hideBlock');
							$("#dialog-editFunctions .errorDiv label").text(
									'Please add a store to save.');
						}
					});
	$('#exportmultistoreButton').unbind('click');
	$('#exportmultistoreButton')
			.click(
					function() {
						$("#dialog-editFunctions .errorDiv ").addClass('hideBlock');
						if ($('#dialog-editFunctions .ContentTable tbody tr').length > 1) {
	...
	$('#addstore-popup-cancel-btn').unbind('click');
...
function exportSql(){
	...
	var verifiedStores=[];
	$('#dialog-editFunctions .ContentTable tbody tr').each(function(){
		verifiedStores.push($(this).data('storeinfo'));
	});
	searchParam.verifiedStores=verifiedStores;
...
function replicateSettings(flag){
	var list=[];
	$('#searchResultsHold input[type="checkbox"]:checked').each(function(){
		var data=$(this).attr('data');
		list.push($.parseJSON(data));
	...
	$.ajax({
	    ...
	    beforeSend: function() {
		  ...
		  responseO = $.parseJSON(response);
			//console.log(JSON.stringify(responseO));	
			if(responseO != undefined && responseO.type=='Y' ){
				$.fn.showCustomMsg(['Settings has been replicated to Store('+(responseO.verifiedStores.length>0?toListString(responseO.verifiedStores):responseO.toSite)+') successfully.'],success,'Pilot Strategy');
				$("#dialog-editFunctions").dialog('close');
			}else{
				if(responseO != undefined && responseO.length <= 0 ){
					$.fn.showCustomMsg([responseO.msg],error,'Pilot Strategy');
...
function callVerifyService(recvParam){
	var getAllAccessURL="verifyStores.htm";
	console.log(getAllAccessURL + ' ' + JSON.stringify(recvParam));
	searchParam=recvParam;
	recvParam.results=[];
	recvParam.verifiedStores=[];
	$.ajax({
	    type: "POST",
	    url: getAllAccessURL,
	    data: recvParam,
	    beforeSend: function() {
		  ...
		  if(responseO != undefined){
				if(responseO.verifiedStores!=null && responseO.verifiedStores!=undefined && responseO.verifiedStores.length>0){
					buildVerifiedStoresList(responseO.verifiedStores);
				}else{
					$.fn.showCustomMsg(['Please enter valid store.'],error,'Pilot Strategy');
				}
			}else{
				$('.pageActions').addClass('hideBlock');
				if(responseO != undefined && responseO.length <= 0 ){
					...
					$.fn.showCustomMsg(['Falied to verify store(s)'],error,'Pilot Strategy');
					$("#noRecFoundDiv").addClass("hideBlock");
				}
			}	
	  }).fail(function() {		  
...
function buildVerifiedStoresList(verifiedStoresList){
	for(var i in verifiedStoresList){
		if($('#dialog-editFunctions .ContentTable tbody #'+verifiedStoresList[i].siteNo).length==0){
			$("#dialog-editFunctions .ContentTable tbody").append('<tr id="'+verifiedStoresList[i].siteNo+'"> <td>'+verifiedStoresList[i].siteNo+' - '+verifiedStoresList[i].siteName+'</td> <td class="centerValue"><label class="linkBtn" id="DeleteExpiryRecord-1"> <label class="deleteRecord" site='+verifiedStoresList[i].siteNo+'>Delete</label> </label></td> </tr>');
			$('#dialog-editFunctions .ContentTable tbody #'+verifiedStoresList[i].siteNo).data('storeinfo',verifiedStoresList[i]);
			$('#dialog-editFunctions .ContentTable tbody #'+verifiedStoresList[i].siteNo+' .deleteRecord').unbind('click');
			$('#dialog-editFunctions .ContentTable tbody #'+verifiedStoresList[i].siteNo+' .deleteRecord').click(function(){
				$('#dialog-editFunctions .ContentTable tbody #'+$(this).attr('site')).remove();
				if($("#dialog-editFunctions .ContentTable tbody tr").length<=0){
					$('#dialog-editFunctions .ContentTable').addClass('hideBlock');
					$("#dialog-editFunctions #createOrderButton,#exportmultistoreButton").addClass('hideBlock');
				}else{
					$('#dialog-editFunctions .ContentTable').removeClass('hideBlock');
					$("#dialog-editFunctions #createOrderButton,#exportmultistoreButton").removeClass('hideBlock');
				}
			});
		}
	}
