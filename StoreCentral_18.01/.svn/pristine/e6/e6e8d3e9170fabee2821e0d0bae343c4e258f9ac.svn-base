...
$(document)
		.ready(
				function() {
					...
					var newDate1 = $yesterday.getDate();
					var newMonth1 = $yesterday.getMonth()+1; //January is 0!
					var newYear1 = $yesterday.getFullYear();
					if (newDate1 < 10) {
						newDate1 = '0' + newDate1;
					}
					if (newMonth1 < 10) {
					...
					var yesterday1 = (newDate1 + "/" + (newMonth1) + "/" + newYear1);
					$('#dateFrom').val(yesterday1);
		...
		if (consolidation != null) {
			...
			//for ( var m in consolidation) {
				...
				for ( var i = 0; i < list.length; i++) {
					...
					if (Number(list[i].salesTender) != 0
...
//	});
		...
		if (list != undefined && list != null && list.length > 0) {
			...
			if (/*Number(openLoan) != 0 ||*/ Number(saleTen) != 0
				...
				storeTotalTender += ' <tr class="totVal graytd valueInfo appendStore hideStoreTotal hideBlock">'
						...
						+ list[0].tenderTypeT
						+ '</td>'
						+ '<td class="numberColumn rightValue valueInfo noSort openLTotal hideBlock">'
						...
						+ Number(pickUp).toFixed(2)
						+ '</td>'
						+ '<td class="numberColumn rightValue noSort   lastColumn valueInfo  hideBlock">'
	...
	storeTotalTender += ' <tr class="totVal graytd valueInfo appendStore hideStoreTotal hideBlock">'
			...
			+ '<td class="numberColumn rightValue valueInfo noSort finalOpenLoanTotal hideBlock">0.00'
			+ '</td>'
			+ '<td class="numberColumn rightValue valueInfo noSort   ">'
...
function posConsolidationJasper() {
	...
	if(isNotJasperPrintValid()) {
		...
		$('#ALHFinancialData').attr("action", "getALHFinancialData.pdf");
