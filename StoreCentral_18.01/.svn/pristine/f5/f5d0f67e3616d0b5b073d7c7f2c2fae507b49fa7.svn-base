			...
			<table cellspacing="0" class="ContentTable treetable drilldownTable"
				...
				<c:forEach items="${articleSearchResutlsList}" var="articlelist">
					<tr id="row-<c:out value="${count}"></c:out>">
						...
						<td id="refNo-<c:out value="${count}"></c:out>" class="article-no">${articlelist.articleNo} <c:if test="${not empty articlelist.packBreakDownArticleNo}">(</c:if>${articlelist.packBreakDownArticleNo}<c:if test="${not empty articlelist.packBreakDownArticleNo}">)</c:if></td>
						<td>${articlelist.description}</td>
						<td id="uom-<c:out value="${count}"></c:out>"  class="article-uom">${articlelist.uom}</td>
						<td id="qty-<c:out value="${count}"></c:out>" class="<c:if test="${articlelist.saveFlag=='N'}">hideBlock</c:if>">${articlelist.storeReasonCodeDesc}</td>
						<td id="qtyEdit-<c:out value="${count}"></c:out>"
							class="<c:if test="${articlelist.saveFlag=='Y'}">hideBlock</c:if> saveCheck"><select name="aQMReasonCode" value="${articlelist.storeReasonCode}" class="selectOptions"
						...
						<td id="comments-<c:out value="${count}"></c:out>" class="<c:if test="${articlelist.saveFlag=='N'}">hideBlock</c:if>">${articlelist.lineItemComments}</td>
						<td id="commentsEdit-<c:out value="${count}"></c:out>"
							class="aqm-input <c:if test="${articlelist.saveFlag=='Y'}">hideBlock</c:if>"><input maxlength="40" type="#" value="${articlelist.lineItemComments}"
						...
						</label><label class="linkBtn saveRowBtn <c:if test="${articlelist.saveFlag=='Y'}">hideBlock</c:if>"
