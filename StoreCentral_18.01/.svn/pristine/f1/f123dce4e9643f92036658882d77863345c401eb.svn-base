...
<link rel="stylesheet" href="../../styles/jquery.treetable.theme.default.css?version=${properties.version}" />
...
<script>
	$(document).ready(function() {
		...
		.click( 
				function() {
					$('.tableInfoError').addClass('hideBlock');//added for advance search click defect
		...
		<div class="headWrapper">
			...
			<div class="breadcrumbWrapper">
				...
				<div class="statusWrapper">
					<label class="loading hideBlock"  id="statusImg">We are getting data, please wait</label>
			 
				<c:if test="${not empty articleSearchResutlsList}"><label class="secondaryActionBtn" onclick="">Clear Results</label></c:if>
			...
			<div class="lookupParamWrapper">
				<form id="search" action="requestSearch.htm" method="post">
					<input type="hidden" name="siteNo" id="siteNo"
						value="${user.siteNo}" /><input type="hidden" name="hierarchyText" id="hierarchyText"
						value="" /> <input type="hidden" name="hierarchySearchValue" id="hierarchySearchValue"
						value="" /><input type="hidden" name="salesOrg"
						id="salesOrg" value="${user.salesOrg}" /> <input type="hidden"
						id="listCount" name="listCount" value="${model.param.recordCount}" />
					<input type="hidden" id="pageNumber" name="pageNumber"
						value="${model.param.pageNumber}" /> <input type="hidden"
						id="option" value="${model.param.option}" /> <input type="hidden"
						id="index" name="index" /> <input type="hidden"
						id="iv_parent_node" name="iv_parent_node" /> <input type="hidden"
						id="deptIndex" name="deptIndex" /> 
						<input type="hidden" id="rangedFlag" name="rangedFlag" value="" /> 
						
						
						<div class="searchBox">
						<input type="#"
						value="${model.param.value}" placeholder="Search article by" class="textbox textboxDefaultText focus"
						name="value" id="value" maxlength="20"> 
                        </div>
                       <label class="actionBtn" id="searchBtn">Go</label>
                       <!-- class="actionBtn ${properties.Reviewlookupresults}"applicationSettings CR--> 
					<div class="searchByOptions">
						<input type="radio" name="searchByOptions" value="number" class="lookupRadio"
							id="number"><label for="number" class="labelText">Number</label>
						<input type="radio" name="searchByOptions" value="description" class="lookupRadio"
							id="description"><label for="description"
							class="labelText">Description</label> <input type="radio" class="lookupRadio"
							name="searchByOptions" value="reference" id="reference"><label
							for="reference" class="labelText">EAN / TUN / PLU</label>
					</div>
					...
					<div class="advancedParam hideBlock advancedSearchFormat"
						...
						<div class="parameter">
							...
							<input type="radio" name="sourceSupply" value=""
								id="All" checked><label for="warehouse"
								class="labelText">All</label>
							<input type="radio" name="sourceSupply" value="2"
								id="warehouse"><label for="warehouse"
								class="labelText">Warehouse</label> <input type="radio"
								name="sourceSupply" value="1" id="vendor"><label
...
<input type="hidden"
						name="suppNo" id="suppNo" value="${model.param.suppNo}" /> 
						<!--  value="${model.param.supplier}" -->
						<div class="parameter">
							<h3>Supplier:</h3>
							<input type="#" class="textbox"
								placeholder="Enter supplier no. or name" name="supplier" maxlength="20" id="supplier" readonly="readonly"> <label
								class="linkBtn" ><label class="advancedSearch" id="verifySupplier">Verify</label></label>
						</div>
						<!-- End of parameter -->

						<div class="parameter hideBlock">
							<h3>Supplier Name:</h3>
							<input type="#" class="textbox" id="samplePopupTest" maxlength="20">
						...
						<div class="parameter">
								...
								<c:if test="${model.param.rangeFlag=='N'}">
								<input type="checkbox"  name="ranged"  id="rangedChk" value="${model.param.rangeFlag}"><label for="ranged" class="labelText">Ranged Articles Only</label>
								</c:if>
								<c:if test="${model.param.rangeFlag=='Y'}">
								<input type="checkbox"  name="ranged"  id="rangedChk" value="${model.param.rangeFlag}" checked><label for="ranged" class="labelText">Ranged Articles Only</label>
								</c:if>
							</div> <!-- End of parameter -->
							

						<label class="actionBtn" id="tooManysupplier">Go</label>
						 <!-- class="actionBtn ${properties.Reviewlookupresults}"applicationSettings CR-->
			...
			<div class="lookupActionWrapper">
				<label class="linkBtn" id="advLink1"><label
					class="advancedSearch  ${properties.Advancedarticlesearch}">Advanced Search</label></label> <label
		...
		<div class="ContentTableWrapper">
			<div class="tableInfo  tableInfoError hideBlock">
				...
				<c:if test="${not empty noData || not empty articleSearchResutlsList}">
			...
			<div class="tableInfo">
				...
				<div class="tableTitle">
					...
					<input type="hidden" id="recordListFlag" value="1"/>
						<h4>
							Total <strong>${model.param.recordCount}</strong> results found
							for '<strong class="searchString"><c:if
									test="${not empty model.param.value}">
						${model.param.value}
						</c:if> <c:if test="${not empty model.param.suppNo}">
						${model.param.suppNo}&nbsp;${model.param.suppName}
						</c:if>
						<c:if test="${not empty model.param.hierarchyText}">
				...
				<!-- End of table title -->
				<c:if test="${not empty articleSearchResutlsList && model.param.recordCount>20 }">
			...
			<c:if test="${not empty articleSearchResutlsList}">
				<table cellspacing="0" class="sortTable ContentTable actionRows">
					...
					<tr>
						...
						<th data-sort="string" class="sorted ascending">Article Description</th>
					...
					<tbody>
					<c:forEach items="${articleSearchResutlsList}" var="articleInfo">
					
		
						<!-- <tr  onclick="navigateToDetail(<%=i++%>);" class="${properties.ViewItemdetails}">application settings CR-->
						<tr  onclick="navigateToDetail(<%=i++%>);" class="">
				...
				<div class="tableFooter">
					<div class="legend">
						<label> Legend: <label class="positiveStatus">Ranged</label><label class="negativeStatus">Not Ranged</label><label>
					</div>
					
					<c:if test="${ model.param.recordCount>20 }">
					<div class="paginationWrapper bottomPagination" id="paginationDiv2">
						<div class="pagination-holder clearfix">
							<div id="compact-pagination"
								class="compact-theme simple-pagination"></div>
						</div>
				...
				<div class="hierarchyWrapper hierarchySearchDiv">
					...
					<div class="hierarchyContent" id="deptDiv">				
						...
						<div class="hierarchyList">
							...
							<input type="hidden" id="child<%=j%>"
								...
								<c:if test="${deptInfo.level=='1'}">
										...
										<li >
											<input class="department" type="radio" name="departmentList" value="${deptInfo.node}" data-tt-id="<%=j%>" id="${deptInfo.node}">
											<label for="${deptInfo.node}" class="labelText">${deptInfo.nodeDesc}</label></li>
						...
						<div class="heirachyBottom">
							<span class="totalCount">
								<label>Total:<strong id="deptLstCnt"></strong></label>
							</span>							
					...
					<div class="hierarchyContent" id="subCatDiv">				
						...
						<div class="heirachyBottom">
							...
							<span  class="heirachyAction hideBlock">
					...
					<div class="hierarchyContent lastContent" id="segDiv">				
						...
						<div class="heirachyBottom">
							...
							<span id="segmentBtn" class="heirachyAction hideBlock">
								<label class="actionBtn">Go</label>
							</span>	
	...
	<div id="dialog-modal" title="Verify Supplier">
		<div class="popupContent">
			<div class="popupSearchWrapper" id="popupSearch">
				<h3>Supplier Name:</h3>
				<input type="#" placeholder="Enter supplier name" class="textbox textboxDefaultText" id="vendorDesc" maxlength="20">
				<label class="actionBtn" id="goButtonSample">Go</label>
			</div> <!-- End of popup search wrapper -->
			
			<div class="popupData" id="popupDataDiv">
