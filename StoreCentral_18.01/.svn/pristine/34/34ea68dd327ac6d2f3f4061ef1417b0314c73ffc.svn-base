			...
			.widget(
					"db.jWizard",
					{
						...
						_destroySteps : function() {
							this.$steps.show().unwrap().unwrap() // Unwrap 2x: .jw-steps-wrap + .jw-content
						...
						_updateButtons : function() {
							...
							case $steps.length - 1:
								...
								if(validateFields()){
									$('[name="rpl_roleAll"]').prop('checked',false);
									$.ajax({
							            type: "get",
							            url: "getRoles.htm",
							            data: {							          
							              salesOrg: $('[name="rpl_saleorg"]:checked').val()
							            },
							            beforeSend: function() {
							              startLoading();
							            },
							            success: function(response){							            	
							            	...
							            	var roles=data.data;
							            	var content='';
							            	var sorg=$('[name="rpl_saleorg"]:checked').val()
							            	for(var i in roles){
							            		if(!(currentSaleOrg==sorg && roles[i].code==currentRoleId))
							            		content+='<tr><td class=" centerValue"><input type="checkbox" name="rpl_role" value="'+roles[i].code+'"></td><td class="lastColumn">'+roles[i].desc+'</td></tr>';
							            	}
							            	$('#relicate-roles').html(content);
							            	bindRolesCheckBoxEvent();
							            	$prev.show();
											$prev.attr('onclick','hideError();');
											$next.hide();
											$finish.show();
...
function validate() {
	...
	if ($('#receiveAmount').is(':checked')) {
		...
		if ($('#invoiceConfirm').val() == "") {
			...
			$('.errorDiv h4').text(
					'Please re-enter invoice number for confirmation');
			$('#invoiceConfirm').focus();
		} else if ($('#invoice').val() != $('#invoiceConfirm').val()) {
			$('#next').removeClass('jw-button-next');
			$('.errorDiv h4').text(
					'Invoice number doesnt match with basic information');
			$('#invoiceConfirm').focus();
		} else if ($('#amountConfirm').val() == "") {
			$('#next').removeClass('jw-button-next');
			$('.errorDiv h4').text(
					'Please re-enter total amount for confirmation');
			$('#amountConfirm').focus();
		} else if (Number($('#amount').val()) != Number($('#amountConfirm')
				.val())) {
			$('#next').removeClass('jw-button-next');
			$('.errorDiv h4').text(
					'Total amount doesnt match with basic information');
			$('#amountConfirm').focus();
		...
		if ($('#docketConfirm').val() == "") {
			...
			$('#next').removeClass('jw-button-next');
			$('.errorDiv h4').text(
					'Please re-enter delivery docket for confirmation');
			$('#docketConfirm').focus();
			...
			$('.errorDiv h4')
					.text(
							'Delivery docket number doesnt match with the basic information');
			$('#docketConfirm').focus();
...
function validateFields() {
	$('#dialog-replicate #repicate-err').remove();
	if($('[name="rpl_saleorg"]:checked').length==1){
		$('#next').addClass('jw-button-next');
	}else{
		$('<div class="errorDiv parameter replicate-error" id="repicate-err"><label>Please select any sales org to replicate.</label></div>').insertAfter('#dialog-replicate #cancel')
		return false;
	}
	return true;
}
