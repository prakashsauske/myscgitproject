	...
	$
			.extend({
				tablesorter : new function() {
					...
					this.defaults = {
						cssHeader : "header",
						cssAsc : "descending sorted",
						cssDesc : "ascending sorted",
					...
					function multisort(table, sortList, cache) {
						//table=changeTbl(table);
						$(table).children('tbody:first').find('tr').removeClass('hideBlock').show();
						if (table.config.debug) {
...
function reDOPagination(tbl){
	var i = 1;
	var cnt = 1;
	var sortvalFlag=false;
	var paginationFlag=false;
	var paginationDiv='';
	if($(tbl).hasClass('.sortval'))
		sortvalFlag=true;
	if($(tbl).parent().find('.paginationDiv').length>0)
		{
		paginationFlag=true;
		paginationDiv=$(tbl).parent().find('.paginationDiv');
		}
	...
	//if($('#printbody').children().length ==0)
	{
		var deptSalesTax = '';
		var output = '';
